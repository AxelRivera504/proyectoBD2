...
BEGIN
    ...
    INSERT INTO InventarioKardex(IdProducto,Movimiento,Cantidad,Saldo,Observaciones,Fecha)
    SELECT d.IdProducto,'Entrada Devolucion',d.Cantidad,(SELECT Existencia FROM Producto WHERE IdProducto=d.IdProducto),'Devoluci�n a proveedor',GETDATE()
    FROM inserted d;
END
GO
...
BEGIN
    ...
    INSERT INTO InventarioKardex(IdProducto,Movimiento,Cantidad,Saldo,Observaciones,Fecha)
    SELECT d.IdProductoInsumo,'Salida Elaboracion',d.CantidadUsada,(SELECT Existencia FROM Producto WHERE IdProducto=d.IdProductoInsumo),'Uso en elaboraci�n',GETDATE()
    FROM inserted d;
END
GO
...
-- 8) OrdenCompraDetalle UPDATE (recepci�n) -> registrar entrada y ajustar existencia
...
BEGIN
    ...
    BEGIN
        ...
        SELECT d.IdProducto,'Entrada OrdenCompra',d.CantidadRecibida,
               ...
               CONCAT('Recepci�n OC ',d.IdOrdenCompra),GETDATE()
        FROM inserted d
